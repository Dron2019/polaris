...
let inputDefaultText = (form) => {
    ...
    //         e.target.value = '+(380)';
    //     })
    let symbolRegExp = /^.*[^A-zА-яЁё].*$/g;
    document.querySelector(form).querySelector('input[name=telephone]').addEventListener('keyup', function() {
        document.querySelector(form).querySelector('input[name=telephone]').value = document.querySelector(form).querySelector('input[name=telephone]').value.replace(symbolRegExp, '');
